...
assert server["headers"]["Authorization"] == "Bearer ${METATATE_CORTEX_PAT}"
assert server["headers"]["X-Snowflake-Authorization-Token-Type"] == "PROGRAMMATIC_ACCESS_TOKEN"
assert server["headers"]["X-Snowflake-Role"] == "METATATE_CORTEX_USER"
'

oauth_dry_run_output="$(
  "${repo_root}/bin/metatate-cortex-mcp-add" \
    --auth oauth \
    --account-url https://example.snowflakecomputing.com \
...
DRY_RUN_OUTPUT="${oauth_dry_run_output}" python3 -c 'import json, os
server = json.loads(os.environ["DRY_RUN_OUTPUT"])
